...
import static io.github.jristretto.testhelpers.TestHelpers.*;
...
public class NFTestHelpers {
    ...
    public static void checkField( Class<?> targetClass,
        ...
        try {
            ...
            _assertEquals( "field " + fieldName + " should be of type "
                    + fieldType,
                    fieldType,
                    f.getType() );
            int fieldModifiers = f.getModifiers();
            if ( ( modifierMask & fieldModifiers ) != modsRequired ) {
                _fail( "field '" + f.getName()
                        + "' should be declared '"
    ...
    public static void checkFieldAndNaming( Class<?> targetClass,
        ...
        if ( ( PUBLIC | STATIC | FINAL ) == modifiers ) {
            ...
            _assertEquals( "first char not lower case",
...
//        _assertTrue( , success );
    ...
    public static void assertStringMatchesAll( String target,
        ...
        _assertTrue( "the string '" + target + "' does not match " + notMatching, success );
    }
    ...
    public static void assertIsOrImplements( String message, Type candidate,
        ...
        _assertTrue( message, checkIsOrImplements( candidate, expectedInterface ) );
    }
    ...
    public static <FT> FT getFieldValueByType( Class<?> clz, Object instance,
        ...
        try {
            ...
            _assertTrue( "Field of type " + fieldType + " is missing", 0
                    < fields.length );
    ...
    public static <T> void assertContainsExactlyUsingEquals( String msg,
        ...
        _assertTrue( msg + missing + " extra :" + notWanted, allFound && noUnWanted );
    }
...
//        _fail( "stop" );
    ...
    public static <T> void assertEqualsAndHashCode( T ref, T equal, T... unEqual ) {
        ...
        _assertFalse( "equals ref " + ref.toString() + " and null should produce false", ref.equals( t0 ) );
        _assertFalse( "equals to other type should produce false", ref.equals( object ) );
        _assertTrue( "equals with self", ref.equals( ref ) );
        _assertTrue( "ref and equal object should report equal", ref.equals( equal ) );
        _assertTrue( "equal and ref object should report equal", equal.equals( ref ) );

        for ( int i = 0; i < unEqual.length; i++ ) {
            T ueq = unEqual[ i ];
            _assertFalse( "ref " + ref.toString() + "\n and other unequal[" + i
                    + "]: " + ueq.toString() + "\n should be unequal" + ref, ref.equals( ueq ) );
        }

        _assertEquals( "equals objects " + ref.toString() + "and " + equal.toString() + "should have same hashcode", ref.hashCode(), equal.hashCode() );
    }
